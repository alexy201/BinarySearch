...
public int recursiveLinearSearch(int catNumToFind, int startIndex)
{
  ...
  if (catNumToFind == store[startIndex].getCatNum()){return store[startIndex].getInventory();}
  return recursiveLinearSearch(catNumToFind, startIndex + 1);
}
public int binarySearch(int catNumToFind)
{
  int l = 0, r = store.length;
  while (l < r){
    int mid = (int)Math.floor((l + r)/2.0);
    if (store[mid].getCatNum() == catNumToFind){return store[mid].getInventory();}
  ...
  if (l >= store.length || store[l].getCatNum() != catNumToFind){return -1;}
  return store[l].getInventory();
}
public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh)
{
  if (nLow > nHigh){return -1;}
  int mid = (int)Math.floor((nLow + nHigh)/2.0);
  if (store[mid].getCatNum() == catNumToFind){return store[mid].getInventory();}
  if (store[mid].getCatNum() > catNumToFind){return recursiveBinarySearch(catNumToFind, nLow, mid - 1);}
  else {return recursiveBinarySearch(catNumToFind, mid + 1, nHigh);}
}
public void setup()
{
